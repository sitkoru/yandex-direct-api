...
class KeywordsResearchService extends BaseService
{
    ...
    public function deduplicate(array $DeduplicateRequestItems, ?array $DeduplicateOperation = null)
    {
        ...
        return new DeduplicateActionResult([
            'Add'     => $this->mapArray($result->Add ?? [], DeduplicateResponseAddItem::class),
            'Update'  => $this->mapArray($result->Update ?? [], DeduplicateResponseUpdateItem::class),
            'Delete'  => $this->mapArray($result->Delete ?? [], IdsCriteria::class),
            'Failure' => $this->mapArray($result->Failure ?? [], DeduplicateErrorItem::class),
        ]);
    }
